...
class MyAliPay(View):
    ...
    def get_trade_url(self, order_id, amount):
        # pc网站支付地址
        # 沙箱的支付地址  https://openapi.alipaydev.com/gateway.do?+querystring
...
class ResultView(MyAliPay):
    def get(self,request):
        ...
        if is_verify:
            # 消息来源可靠
            order_id=request_data.get('oute_trade_no')
            # 去自己数据库中查询该订单的状态 例如dadashop中订单状态案例，此时订单状态应该进入到‘待发货/已付款’
            if ORDER_STATUS==2:
                ...
                if res:
                    ...
                    return HttpResponse('主动查询得知未支付成功')
        else:
            return HttpResponse('非法访问')

    def post(self,request):
        # notify_url的处理。支付宝发表单post请求将支付结果告知商户
