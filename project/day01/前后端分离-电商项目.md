...
redis-hash结构
	...
	哈希碰撞:链表式,类似于一个位置存储一个列表,

	扩容:hash元素个数等于第一维数组的长度时出发,为两倍

	由于redis本身单进程单线程,故redis采用保留两份座位表的方式,逐步进行迁移的方式,去避免作为重排时导致的redis阻塞问题
```
...
```
tar -zxvf django-cores-headers-3.0.2.tar.gz解压安装包
cd  django-cores-headers-3.0.2移动目录到解压的路径下
